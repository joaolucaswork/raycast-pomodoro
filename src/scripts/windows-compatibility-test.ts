...
export async function runWindowsCompatibilityTest(): Promise<WindowsCompatibilityReport> {
  ...
  try {
    ...
    if (!report.platform.isWindows) {
      console.log('⚠️ Warning: Not running on Windows. Some tests may not be applicable.');
    }
    ...
    console.log('Please switch between different Windows applications for 20 seconds');
    ...
    report.applicationSupport.successfullyTracked = usage.filter(app => app.timeSpent > 0).length;
    ...
    report.performance.averageDetectionTime = validDetectionTimes.length > 0 
      ? validDetectionTimes.reduce((sum, time) => sum + time, 0) / validDetectionTimes.length
      : 0;
    
    console.log(`Applications tracked: ${report.applicationSupport.totalApplicationsTested}`);
    console.log(`Successfully tracked: ${report.applicationSupport.successfullyTracked}`);
    console.log(`Tracking accuracy: ${stats.trackingAccuracy}%`);
    console.log(`Health status: ${health.isHealthy ? 'Healthy' : 'Issues detected'}`);
    console.log(`Average detection time: ${report.performance.averageDetectionTime.toFixed(2)} ms`);
    ...
    console.log(`Common Windows apps detected: ${detectedCommonApps.length}/${commonWindowsApps.length}`);
    ...
    console.log(`Estimated Memory Usage: ${report.performance.memoryUsage.toFixed(2)} MB`);

    // Generate Recommendations
    console.log('\n💡 Generating Recommendations');
    
    if (!report.apiCompatibility.getFrontmostApplicationWorks) {
      report.recommendations.push('Raycast API compatibility issues detected. Update Raycast to the latest version.');
    }
    
    if (report.applicationSupport.successfullyTracked < report.applicationSupport.totalApplicationsTested * 0.8) {
      report.recommendations.push('Low application tracking success rate. Consider increasing tracking interval.');
    }
    
    if (report.performance.averageDetectionTime > 50) {
      report.recommendations.push('High detection latency detected. Consider optimizing tracking frequency.');
    }
    
    if (health.errorCount > 3) {
      report.recommendations.push('Multiple tracking errors detected. Check Windows permissions and antivirus settings.');
    }
    
    if (detectedCommonApps.length < 3) {
      report.recommendations.push('Few common Windows applications detected. Test with more diverse applications.');
    }
    
    if (report.performance.cpuImpact === 'High') {
      report.recommendations.push('High performance impact detected. Consider reducing tracking frequency or optimizing code.');
    }
    
    if (report.recommendations.length === 0) {
      report.recommendations.push('Excellent Windows compatibility! No issues detected.');
    }

    // Final Report
    console.log('\n📊 Windows Compatibility Report Summary');
    console.log(`Platform: ${report.platform.isWindows ? 'Windows ✅' : 'Other Platform ⚠️'}`);
    console.log(`API Compatibility: ${report.apiCompatibility.getFrontmostApplicationWorks ? 'Working ✅' : 'Issues ❌'}`);
    console.log(`Applications Tracked: ${report.applicationSupport.successfullyTracked}/${report.applicationSupport.totalApplicationsTested}`);
...
function calculateCompatibilityScore(report: WindowsCompatibilityReport): number {
  let score = 0;
  ...
  const successRate = report.applicationSupport.totalApplicationsTested > 0 
    ? (report.applicationSupport.successfullyTracked / report.applicationSupport.totalApplicationsTested) * 100
    : 0;
