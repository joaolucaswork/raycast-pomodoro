...
export async function runPerformanceTests(): Promise<PerformanceMetrics> {
  ...
  try {
    ...
    metrics.accuracy.errorRate =
      health.errorCount > 0 ? (health.errorCount / 30) * 100 : 0; // Errors per second as percentage
    ...
    const memoryEfficient =
      metrics.memoryUsage.peak - metrics.memoryUsage.initial < 10; // Less than 10MB increase
